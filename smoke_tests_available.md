# Caliptra Tests Available For `simulate.sh`

Source repo: `hack_dac_26/third_party/caliptra-rtl`

Local Caliptra HEAD:

```text
a687e263ab4550b40ab428dee1494a07a9add7d5
```

This file is generated from the local checkout by scanning Caliptra integration stimulus YAMLs and applying the same support filter used by `./scripts/simulate.sh --list`.

A test is listed as supported when it is stimulus-referenced and has a same-name C/assembly source, same-name hex, or per-test makefile. Prebuilt-only ELF tests are excluded because this older Caliptra Makefile can generate invalid `dccm.hex` addresses for them.

Run any supported test with:

```bash
./scripts/simulate.sh --test <TESTNAME>
```

The default remains:

```bash
./scripts/simulate.sh --test iccm_lock
```

Supported count: 117 tests (14 non-`smoke_test_*`, 103 `smoke_test_*`).

Excluded prebuilt-only count: 34.

## Regression Status — 2026-07-20

**Use only the passing list below for future smoke-test validation.** The parallel regression started all 103 `smoke_test_*` tests with 45 workers. Results: 78 passed, 24 failed, 1 stalled and was stopped.

### Passing Smoke Tests — Approved For Use

```text
smoke_test_abr_kv_zero_error
smoke_test_aes_gcm
smoke_test_aes_kv_out_rand
smoke_test_ahb_mux
smoke_test_cg_wdt
smoke_test_clk_gating
smoke_test_cshake
smoke_test_datavault_basic
smoke_test_datavault_reset
smoke_test_dma_aes_gcm
smoke_test_dma_aes_gcm_long
smoke_test_dma_aes_gcm_non_gcm_en_dec
smoke_test_dma_aes_gcm_short_1_dword
smoke_test_dma_aes_gcm_short_dword
smoke_test_dma_aes_kv
smoke_test_doe_cg
smoke_test_doe_kv_ocp_progress
smoke_test_doe_rand
smoke_test_doe_scan
smoke_test_ecc_errortrigger1
smoke_test_ecc_errortrigger2
smoke_test_ecc_errortrigger3
smoke_test_ecc_errortrigger4
smoke_test_ecc_keygen
smoke_test_ecc_sign
smoke_test_ecc_verify
smoke_test_ecdh
smoke_test_fw_kv_backtoback_hmac
smoke_test_hmac
smoke_test_hmac_errortrigger
smoke_test_hw_config
smoke_test_iccm_reset
smoke_test_kv
smoke_test_kv_cg
smoke_test_kv_hmac_flow
smoke_test_kv_hmac_multiblock_flow
smoke_test_kv_lock_use_mid_read
smoke_test_kv_mldsa
smoke_test_kv_mlkem
smoke_test_kv_parallel_access
smoke_test_kv_rules_ocp_lock
smoke_test_kv_securitystate
smoke_test_kv_swwe_lock
smoke_test_kv_uds_reset
smoke_test_mbox
smoke_test_mbox_byte_read
smoke_test_mbox_cg
smoke_test_mldsa
smoke_test_mldsa_edge
smoke_test_mldsa_errortrigger
smoke_test_mldsa_externalmu
smoke_test_mldsa_externalmu_keygen_sign_vfy_rand
smoke_test_mldsa_keygen_sign_vfy_rand
smoke_test_mldsa_keygen_standalone_sign_vfy_rand
smoke_test_mldsa_kv_ocp_progress
smoke_test_mlkem_all_zero_seed
smoke_test_mlkem_kv_endian
smoke_test_mlkem_shared_key
smoke_test_pcr_signing
smoke_test_pcr_zeroize
smoke_test_ras
smoke_test_sha256
smoke_test_sha256_wntz
smoke_test_sha256_wntz_rand
smoke_test_sha3
smoke_test_sha3_externalmu
smoke_test_sha3_interrupt
smoke_test_sha3_regs
smoke_test_sha512
smoke_test_sha512_restore
smoke_test_sha_accel
smoke_test_sram_ecc
smoke_test_strap
smoke_test_trng
smoke_test_veer
smoke_test_wdt
smoke_test_wdt_rst
smoke_test_zeroize_crypto
```

### Failed Smoke Tests — Do Not Use For Validation

```text
smoke_test_aes_kv_rand
smoke_test_datavault_lock
smoke_test_datavault_mini
smoke_test_dma
smoke_test_dma_aes_gcm_collision_test
smoke_test_dma_aes_gcm_rd_enc_axi_err
smoke_test_dma_aes_gcm_wr_enc_axi_err
smoke_test_ecc_flow1_kv_ocp_progress
smoke_test_ecc_flow2_kv_ocp_progress
smoke_test_hek_flow
smoke_test_hmac_kv_ocp_progress
smoke_test_kv_crypto_flow
smoke_test_kv_crypto_flow2
smoke_test_kv_doe
smoke_test_kv_ecc_flow1
smoke_test_kv_ecc_flow2
smoke_test_kv_ecdh_flow
smoke_test_kv_write_scan_mode
smoke_test_mldsa_all_zero_seed
smoke_test_mldsa_locked_api
smoke_test_mldsa_zeroize
smoke_test_mlkem
smoke_test_mlkem_errortrigger
smoke_test_mlkem_locked_api
```

### Stalled Smoke Tests — Do Not Use For Validation

```text
smoke_test_mlkem_kv_ocp_progress
```

The stalled test was stopped after approximately 48 minutes without log progress. The failed and stalled tests need separate diagnosis before they can be used as validation targets.

## Supported Non-`smoke_test_*` Integration Tests

```text
c_intr_handler
hello_world_iccm
iccm_lock
kv_entry_read_err
memCpy_ROM_to_dccm
memCpy_dccm_to_iccm
mldsa_pcr_inject_failure
pv_hash_and_sign
pv_hash_zeroize
rand_test_dma
randomized_mldsa_invalid_verify
randomized_mlkem_kv
randomized_pcr_ecc_signing
randomized_pcr_mldsa_signing
```

## Supported `smoke_test_*` Integration Tests

```text
smoke_test_abr_kv_zero_error
smoke_test_aes_gcm
smoke_test_aes_kv_out_rand
smoke_test_aes_kv_rand
smoke_test_ahb_mux
smoke_test_cg_wdt
smoke_test_clk_gating
smoke_test_cshake
smoke_test_datavault_basic
smoke_test_datavault_lock
smoke_test_datavault_mini
smoke_test_datavault_reset
smoke_test_dma
smoke_test_dma_aes_gcm
smoke_test_dma_aes_gcm_collision_test
smoke_test_dma_aes_gcm_long
smoke_test_dma_aes_gcm_non_gcm_en_dec
smoke_test_dma_aes_gcm_rd_enc_axi_err
smoke_test_dma_aes_gcm_short_1_dword
smoke_test_dma_aes_gcm_short_dword
smoke_test_dma_aes_gcm_wr_enc_axi_err
smoke_test_dma_aes_kv
smoke_test_doe_cg
smoke_test_doe_kv_ocp_progress
smoke_test_doe_rand
smoke_test_doe_scan
smoke_test_ecc_errortrigger1
smoke_test_ecc_errortrigger2
smoke_test_ecc_errortrigger3
smoke_test_ecc_errortrigger4
smoke_test_ecc_flow1_kv_ocp_progress
smoke_test_ecc_flow2_kv_ocp_progress
smoke_test_ecc_keygen
smoke_test_ecc_sign
smoke_test_ecc_verify
smoke_test_ecdh
smoke_test_fw_kv_backtoback_hmac
smoke_test_hek_flow
smoke_test_hmac
smoke_test_hmac_errortrigger
smoke_test_hmac_kv_ocp_progress
smoke_test_hw_config
smoke_test_iccm_reset
smoke_test_kv
smoke_test_kv_cg
smoke_test_kv_crypto_flow
smoke_test_kv_crypto_flow2
smoke_test_kv_doe
smoke_test_kv_ecc_flow1
smoke_test_kv_ecc_flow2
smoke_test_kv_ecdh_flow
smoke_test_kv_hmac_flow
smoke_test_kv_hmac_multiblock_flow
smoke_test_kv_lock_use_mid_read
smoke_test_kv_mldsa
smoke_test_kv_mlkem
smoke_test_kv_parallel_access
smoke_test_kv_rules_ocp_lock
smoke_test_kv_securitystate
smoke_test_kv_swwe_lock
smoke_test_kv_uds_reset
smoke_test_kv_write_scan_mode
smoke_test_mbox
smoke_test_mbox_byte_read
smoke_test_mbox_cg
smoke_test_mldsa
smoke_test_mldsa_all_zero_seed
smoke_test_mldsa_edge
smoke_test_mldsa_errortrigger
smoke_test_mldsa_externalmu
smoke_test_mldsa_externalmu_keygen_sign_vfy_rand
smoke_test_mldsa_keygen_sign_vfy_rand
smoke_test_mldsa_keygen_standalone_sign_vfy_rand
smoke_test_mldsa_kv_ocp_progress
smoke_test_mldsa_locked_api
smoke_test_mldsa_zeroize
smoke_test_mlkem
smoke_test_mlkem_all_zero_seed
smoke_test_mlkem_errortrigger
smoke_test_mlkem_kv_endian
smoke_test_mlkem_kv_ocp_progress
smoke_test_mlkem_locked_api
smoke_test_mlkem_shared_key
smoke_test_pcr_signing
smoke_test_pcr_zeroize
smoke_test_ras
smoke_test_sha256
smoke_test_sha256_wntz
smoke_test_sha256_wntz_rand
smoke_test_sha3
smoke_test_sha3_externalmu
smoke_test_sha3_interrupt
smoke_test_sha3_regs
smoke_test_sha512
smoke_test_sha512_restore
smoke_test_sha_accel
smoke_test_sram_ecc
smoke_test_strap
smoke_test_trng
smoke_test_veer
smoke_test_wdt
smoke_test_wdt_rst
smoke_test_zeroize_crypto
```

## Excluded Prebuilt-Only ELF Tests

These are referenced by stimulus YAMLs but are not supported by `simulate.sh` in this older pinned commit. Their prebuilt ELF `.data` sections use DCCM VMAs with small LMAs, and the old Makefile transforms LMAs into out-of-range `dccm.hex` addresses for Verilator `$readmem`.

```text
fw_test_lms24
fw_test_lms32
fw_test_lms_n24_w1_h10
fw_test_lms_n24_w1_h15
fw_test_lms_n24_w1_h20
fw_test_lms_n24_w1_h5
fw_test_lms_n24_w2_h10
fw_test_lms_n24_w2_h15
fw_test_lms_n24_w2_h20
fw_test_lms_n24_w2_h5
fw_test_lms_n24_w4_h10
fw_test_lms_n24_w4_h15
fw_test_lms_n24_w4_h20
fw_test_lms_n24_w4_h5
fw_test_lms_n24_w8_h10
fw_test_lms_n24_w8_h15
fw_test_lms_n24_w8_h20
fw_test_lms_n24_w8_h5
fw_test_lms_n32_w1_h10
fw_test_lms_n32_w1_h15
fw_test_lms_n32_w1_h20
fw_test_lms_n32_w1_h5
fw_test_lms_n32_w2_h10
fw_test_lms_n32_w2_h15
fw_test_lms_n32_w2_h20
fw_test_lms_n32_w2_h5
fw_test_lms_n32_w4_h10
fw_test_lms_n32_w4_h15
fw_test_lms_n32_w4_h20
fw_test_lms_n32_w4_h5
fw_test_lms_n32_w8_h10
fw_test_lms_n32_w8_h15
fw_test_lms_n32_w8_h20
fw_test_lms_n32_w8_h5
```
